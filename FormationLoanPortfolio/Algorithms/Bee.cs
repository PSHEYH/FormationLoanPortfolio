...
namespace FormationLoanPortfolio
{
    class Bee
    {
        ...
        public void Move()
        {
            ...
            for (int i = 0; i < Solution.Length; i++)
            {
                if (randomPlace == i)
                {
                    ...
                    short[] posibleSolution = new short[Solution.Length];
                    
                    for (int j = 0; j < Solution.Length; j++)
                    {
                        posibleSolution[j] = Solution[j];
                    }

                    ListOfSolutions.Add(posibleSolution);
                    Solution[i] = Solution[i] == (short)1 ? (short)0 : (short)1;
                }
            }
        }
            ...
            double[] P_j, double a1, double a2, double r, double F)
        {
            ...
            {
                if (Algorithm.F(BestSolution, k_j, t_j, d_j, P_j, a1,a2,r,F) < Algorithm.F(ListOfSolutions[i], k_j, t_j, d_j, P_j, a1, a2, r, F))
                {
